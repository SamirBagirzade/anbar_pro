...
@login_required
@permission_required("inventory.view_stockmovement", raise_exception=True)
def vendor_stock_detail(request):
    warehouse_id = request.GET.get("warehouse", "")
    ...
    purchases = PurchaseLine.objects.select_related("purchase", "purchase__vendor", "item")
    issues = IssueLine.objects.select_related("header", "item")

    if warehouse_id:
        purchases = purchases.filter(purchase__warehouse_id=warehouse_id)
        issues = issues.filter(header__warehouse_id=warehouse_id)
    if item_id:
        purchases = purchases.filter(item_id=item_id)
    ...
    # Build issue qty per item
    issue_totals = {}
    for line in issues:
        issue_totals.setdefault(line.item_id, 0)
        issue_totals[line.item_id] += float(line.qty)

    # FIFO allocation of issues across purchases to estimate remaining qty per vendor
    rows = []
    purchase_groups = {}
    for line in purchases:
        purchase_groups.setdefault(line.item_id, []).append(line)

    for item_id_key, lines in purchase_groups.items():
        remaining_issue = issue_totals.get(item_id_key, 0.0)
        vendor_remaining = {}
        for line in lines:
            qty = float(line.qty)
            consumed = min(qty, remaining_issue)
            remaining_issue -= consumed
            remaining_qty = qty - consumed
            if remaining_qty > 0:
                vendor_remaining.setdefault(line.purchase.vendor, 0.0)
                vendor_remaining[line.purchase.vendor] += remaining_qty
        for vendor_obj, qty_left in vendor_remaining.items():
            rows.append(
                {
                    "item": lines[0].item,
                    "vendor": vendor_obj,
    ...
    return render(
        request,
        "inventory/vendor_stock_detail.html",
        {
            "rows": rows,
            "warehouses": warehouses,
            "items": items_qs,
